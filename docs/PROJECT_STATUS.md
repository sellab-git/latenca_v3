...
3. **[← RESUME HERE] Artur reviews the spec → then `dev-plan` on Phase 1 (single-piece path):** catalogue → PDP → cart → guest checkout, wired to the **data model + POD (Gelato) + payments (`PaymentProvider`/Stripe)**. Phase 2 = wall builder + advisor. Phase 3 = recolor. Keep the **generation seam** (D-020) unused in MVP. Build sequence + full detail in the spec §9.
