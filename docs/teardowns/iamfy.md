...
**Style Quiz (verified — the structured alt on-ramp):** separate mini-app at `shop-chat-agent.iamfy.tech/quiz` (opens in an iframe). Intro: named style archetypes ("Nordic Noir Luxe", "Coastal Wabi"…), "60 seconds", **"No account needed"** (guest). **7 steps**, mixed **multi-select** (Style: Scandinavian/Japandi/Bohemian/Art Deco… + Mood: Calm & Grounded / Inspired & Energised / Sophisticated / Warm & Nostalgic) and **single-select image** questions ("Which room feels most like home?"). Output = a **persistent taste profile** ("always on, remembered every visit") that personalises **collections / search / for-you feed** — NOT a composed wall. So iamfy has **two entry modes: free-text advisor chat (composes a wall) + structured quiz (builds a taste profile for discovery)** — the quiz's "Your style" profile is the chip surfaced in the advisor. **Take:** offer BOTH a free-text brief AND a structured quiz on-ramp (hybrid: quiz for structured prefs, chat for ambiguity), both guest, a **persistent taste profile** feeding personalization. Distinct roles: quiz→discovery, advisor→wall.

**iamfy teardown — COMPLETE for our purposes.** Remaining untested (genuinely negligible value): "Try a brief" examples (same as advisor chat) · room upload/AR (we reject AR) · what creates a saved **Curation** · single-artwork PDP pickers (set PDP already showed the picker model) · mobile layout · empty/error states.
