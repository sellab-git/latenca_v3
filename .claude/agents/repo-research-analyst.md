...
user: "I want to add a new custom hook - what patterns does this codebase use?"
...
user: "Scope: technology, architecture, patterns. We are building a new real-time notification system using Supabase Realtime."
assistant: "I'll run a scoped analysis covering technology detection, architecture, and implementation patterns for the notification system."
...
| `turborepo.json` or `turbo.json` | Turborepo |
...
- **API surface:** If 0.1 found no web framework or server dependency, **and** the root listing shows no API-related directories or files (`routes/`, `api/`, `src/api/`, `supabase/functions/`): skip the API surface category. Report "None detected."
- **Data layer:** Evaluate independently from API surface -- a CLI or worker can have a database without any HTTP layer. Skip only if 0.1 found no database-related dependency (e.g., `@supabase/supabase-js`, prisma, drizzle, knex) **and** the root listing shows no data-related directories (`supabase/`, `prisma/`, `migrations/`, `src/lib/database/`). Otherwise, check the data layer table below.
...
- If the root listing already showed deployment files (e.g., `vercel.json`, `netlify.toml`): read them directly instead of globbing.
...
| `vercel.json`, `netlify.toml`, `render.yaml`, `fly.toml` | Platform deployment |
...
| Route / API directories (`src/api/`, `src/routes/`, `src/services/`) | API routing patterns |
...
| Migration directories (`prisma/migrations/`, `drizzle/`) | Database structure |
| Schema files (`prisma/schema.prisma`, `supabase/migrations/*.sql`) | Data model definitions |
| `supabase/config.toml` | Supabase project configuration |
| RLS policy files in migrations | Row Level Security patterns |

**0.3 Module Structure -- Internal Boundaries**

Scan top-level directories under `src/`, `lib/`, `app/` to identify how the codebase is organized. In monorepos where a specific service was scoped in 0.1b, scan that service's internal structure rather than the full repo.

Common React/Vite SPA structures to look for:
- `src/components/` -- UI components
- `src/hooks/` -- Custom React hooks
- `src/pages/` or `src/routes/` -- Page/route components
- `src/services/` or `src/lib/` -- Business logic and API clients
...
1. **Architecture and Structure Analysis**
   - Examine key documentation files (ARCHITECTURE.md, README.md, CONTRIBUTING.md, and CLAUDE.md only if present for compatibility)
...
3. **Documentation and Guidelines Review**
   ...
   - Document any coding standards or style guides (ESLint, Prettier, etc.)
   - Note testing requirements (Vitest) and review processes
...
5. **Codebase Pattern Search**
   ...
   - Identify common implementation patterns (React component patterns, hook patterns, Supabase client usage)
...
- Deployment model (monolith, multi-service, serverless, etc.)
- API styles in use (REST, GraphQL, Supabase Edge Functions, etc.)
- Data stores and async patterns
- Module organization style
...
- Respect any CLAUDE.md or other project-specific instructions found
